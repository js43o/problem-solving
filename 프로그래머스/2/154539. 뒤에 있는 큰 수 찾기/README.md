...
- **스택** 자료구조를 이용하여 해결할 수 있는 문제이다. 어떻게 적용할 수 있을까?
- 핵심은 비교 연산을 최소화하는 것이다. 각 원소를 차례대로 훑되, 일정한 규칙이 만들어지게끔 해서 '더 비교할 필요가 없는' 상황을 만들 수 있을까?
- 뒤에 나올 큰 수를 찾는 문제이다. 당장 알 수 없는 원소는 keep 해둘 수 있다.

### 성능 요약

메모리: 149 MB, 시간: 58.00 ms
